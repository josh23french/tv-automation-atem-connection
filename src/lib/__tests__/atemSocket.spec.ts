import { CutCommand, ProductIdentifierCommand, InitCompleteCommand, VersionCommand, ProgramInputUpdateCommand, PreviewInputUpdateCommand, ISerializableCommand, BasicWritableCommand } from '../../commands'
...
import { EventEmitterExt } from '../util'
// import { promisify } from 'util'
jest.mock('../atemSocketChild')

export class AtemSocketChildMock extends EventEmitterExt {
	constructor () {
		super()
	}
...
describe('AtemSocket', () => {
	...
	test.only('events', async () => {
		...
		// TODO - something is failing to setup the on listeners, causing all events to fail being setup..

		console.log(AtemSocketChildSingleton)
		...
		await AtemSocketChildSingleton.emit(IPCMessageType.Disconnect)
		await clock.tickAsync(0)
		expect(disconnect).toHaveBeenCalledTimes(1)

		await AtemSocketChildSingleton.emit(IPCMessageType.CommandAcknowledged, { packetId: 'ab', trackingId: 98 })
		await clock.tickAsync(0)
		expect(ack).toHaveBeenCalledTimes(1)
		expect(ack).toHaveBeenCalledWith({ packetId: 'ab', trackingId: 98 })

		await AtemSocketChildSingleton.emit(IPCMessageType.CommandTimeout, { packetId: 'vb', trackingId: 48 })
		await clock.tickAsync(0)
	...
	test('receive - init complete', async () => {
		...
		const pktId = 822
		await AtemSocketChildSingleton.emit(IPCMessageType.InboundCommand, { payloaD: testBuffer, packetId: pktId })
		await clock.tickAsync(0)
	...
	test('receive - multiple commands', async () => {
		...
		const pktId = 822
		await AtemSocketChildSingleton.emit(IPCMessageType.InboundCommand, { payload: Buffer.concat([testCmd1, testCmd2]), packetId: pktId })
		await clock.tickAsync(0)
